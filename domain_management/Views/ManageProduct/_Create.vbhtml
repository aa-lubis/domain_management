...
<script type="text/javascript">
    function submitForm() {
        var button = $("#myModal modal-footer a");
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.result != "success") {
                    ...
                    $("#productlist").prepend(response.html);
                    $("#myModal").modal("hide");
                }
            },
...
@Using Html.BeginForm("Create", "ManageProduct", FormMethod.Post, New With {.id = "formCreateProduct", .onsubmit = "submitForm(); return false;"})
    @<div class="form-horizontal">
        <div class="control-group">
            <div class="control-label">@Html.LabelFor(Function(m) Model.ProductCategory.ProductCategoryName)</div>
            <div class="controls">
                <label style="padding-top: 5px"><strong>@Html.DisplayFor(Function(m) Model.ProductCategory.ProductCategoryName)</strong>@Html.HiddenFor(Function(m) Model.ProductCategoryID)</label>
            </div>
        </div>
        <div class="control-group">
