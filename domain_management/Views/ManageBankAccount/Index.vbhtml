...
@ModelType PagedList.IPagedList(Of domain_management.Entities.BankAccount)
@Section HeadScript
    <script type="text/javascript">
        ...
        function openCreateDialog() {
            ...
            $.ajax({
                url: "@Url.Action("Create", "ManageBankAccount")",
                data: {},
                type: "get",
        ...
        function openEditDialog(id) {
            ...
            $("#myModal").modal({ backdrop: 'static', keyboard: false });
            $.ajax({
                url: "@Url.Action("Edit", "ManageBankAccount")",
                data: { id: id },
                type: "get",
                success: function (response) {
                    $("#myModal .modal-body").html(response);
        ...
        function openDeleteDialog(id) {
            ...
            $("#myModal").modal({ backdrop: 'static', keyboard: false });
            $.ajax({
                url: "@Url.Action("Delete", "ManageBankAccount")",
                data: { id: id },
                type: "get",
                success: function (response) {
                    $("#myModal .modal-body").html(response);
...
<div class="row">
    <div class="span12">
        <fieldset>
            ...
            <a style="margin-bottom: 20px" class="btn" onclick="openCreateDialog()"><i class="icon-plus"></i>&nbsp;Add Bank Account</a>
            @Using Html.BeginForm("Index", "ManageBankAccount", FormMethod.Get, New With {.class = "form-inline"})
                @<div class="input-append pull-right" style="margin-bottom: 10px">
                    @Html.TextBox("filter", Nothing, New With {.class = "span2"})
                    <button type="submit" class="btn"><i class="icon-search"></i></button>
                </div>
            End Using
            <table class="table" id="table_bankaccount">
                <thead>
                    <tr>
                        <th>@Html.LabelFor(Function(m) Model.FirstOrDefault.BankAccountID)</th>
                        <th>@Html.LabelFor(Function(m) Model.FirstOrDefault.Bank)</th>
                        <th>@Html.LabelFor(Function(m) Model.FirstOrDefault.BankAccountName)</th>
                        <th>@Html.LabelFor(Function(m) Model.FirstOrDefault.BankAccountNo)</th>
            ...
            @If Model.PageCount > 0 Then
                ...
                @<div class="pull-right">
                    <div class="pagination" style="margin-top: 5px">
                        <ul>
                            <li class="@IIf(Model.PageNumber = 1, "disabled", "")"><a href="@IIf(Model.PageNumber > 1, Url.Action("Index", "ManageBankAccount"), "#")">&laquo;</a></li>
                    ...
                    If i > Model.PageNumber - span + shiftLeft And i < Model.PageNumber + span + shiftRight Then
                                @<li class="@IIf(i = Model.PageNumber, "active", "")"><a href="@IIf(Model.PageNumber = i, "#", Url.Action("Index", "ManageBankAccount", New With {.p = i}))">@i</a></li>
                    End If
                Next
                            <li class="@IIf(Model.PageNumber = Model.PageCount, "disabled", "")"><a href="@IIf(Model.PageNumber = Model.PageCount, "#", Url.Action("Index", "ManageBankAccount", New With {.p = Model.PageCount}))">&raquo;</a></li>
                        </ul>
                    </div>
                </div>
            End If
